...
train_target = train_PECV.loc[:, ['type_sub', 'type_ins', 'type_del']].mul(train_PECV.Measured_PE_efficiency, axis=0)

test_features = test_PECV.loc[:, ['PBSlen', 'RTlen', 'RT-PBSlen', 'Edit_pos', 'Edit_len', 'RHA_len', 'type_sub',
                                  'type_ins', 'type_del', 'Tm1', 'Tm2', 'Tm2new', 'Tm3', 'Tm4', 'TmD',
                                  'nGCcnt1', 'nGCcnt2', 'nGCcnt3', 'fGCcont1', 'fGCcont2', 'fGCcont3',
                                  'MFE1', 'MFE2', 'MFE3', 'MFE4', 'MFE5', 'DeepSpCas9_score']]
test_target = test_PECV.loc[:, ['type_sub', 'type_ins', 'type_del']].mul(test_PECV.Measured_PE_efficiency, axis=0)

pf_features = train_PF.loc[:, ['PBSlen', 'RTlen', 'RT-PBSlen', 'Edit_pos', 'Edit_len', 'RHA_len', 'type_sub',
                               'type_ins', 'type_del', 'Tm1', 'Tm2', 'Tm2new', 'Tm3', 'Tm4', 'TmD',
                               'nGCcnt1', 'nGCcnt2', 'nGCcnt3', 'fGCcont1', 'fGCcont2', 'fGCcont3',
                               'MFE1', 'MFE2', 'MFE3', 'MFE4', 'MFE5', 'DeepSpCas9_score']]
pf_target = train_PF.loc[:, ['type_sub', 'type_ins', 'type_del']].mul(train_PF.Measured_PE_efficiency, axis=0)
...
def finalize_prediction(pred_, y_):
...
for m in range(n_models):
    ...
    for fold in range(5):
        ...
        for epoch in tqdm(range(n_epochs)):
            ...
            _pred, _y = finalize_prediction(pred_, y_)
            SPR = scipy.stats.spearmanr(_pred, _y).correlation

            if valid_loss < best_score[1]:
                best_score = [train_loss, valid_loss, SPR]

                torch.save(model.state_dict(),
                           'models/' + model_name +'/FM{:02}_auxiliary.pt'.format(fold))

            print('[FOLD {:02}] [M {:03}/{:03}] [E {:03}/{:03}] : {:.4f} | {:.4f} | {:.4f}'.format(fold, m + 1,
                                                                                                   n_models, epoch + 1, n_epochs, train_loss, valid_loss, SPR))
        os.rename('models/' + model_name + '/FM{:02}_auxiliary.pt'.format(fold),
                  'models/' + model_name + '/FM{:02}_{:.4f}.pt'.format(fold, best_score[1]))
...
def finetune_model(model, fold, pf_loader, valid_loader):
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        if SPR > best_score[2]:
            ...
            torch.save(model.state_dict(),
                       'models/' + model_name + '/final/FM{:02}_auxiliary.pt'.format(fold))

        print('FINETUNING: [FOLD {:02}] [E {:03}/{:03}] : {:.4f} | {:.4f} | {:.4f}'.format(
            fold, epoch + 1, n_epochs, train_loss, valid_loss, SPR))

    os.rename('models/' + model_name + '/final/FM{:02}_auxiliary.pt'.format(fold),
              'models/' + model_name + '/final/FM{:02}.pt'.format(fold))
...
for fold in range(5):
    ...
    for (path, dir, files) in os.walk('models/' + model_name + '/'):
    ...
    model.load_state_dict(torch.load('models/' + model_name + '/' + best_model))

    model = finetune_model(model, fold, pf_loader, valid_loader)

    torch.save(model.state_dict(), 'models/' + model_name + '/final/FM{:02}.pt'.format(fold))
...
for fold in range(5):
    ...
    model.load_state_dict(torch.load('models/' + model_name + '/final/FM{:02}.pt'.format(fold)))
...
preds, y = finalize_prediction(preds, y_)

preds = preds * 2.0 + 0.7
y = y * 2.0 + 0.7
print(scipy.stats.spearmanr(preds, y).correlation)

preds = pd.DataFrame(preds, columns=['Predicted PE efficiency'])
preds.to_csv('results/220218_split.csv', index=False)

plot.plot_spearman(preds, y, 'Evaluation of DeepPE2 _ SPLIT OUTPUTS.jpg')
# # %%
