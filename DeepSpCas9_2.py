...
data_train = pd.read_excel('aax9249_table_s1.xlsx', sheet_name=0).iloc[:, [1, 8]]
data_valid  = pd.read_excel('aax9249_table_s1.xlsx', sheet_name=1).iloc[:, [0, 4]]
...
preds = np.zeros((n_models, y_valid.size(0)))
...
for m in range(n_models):
  ...
  for epoch in range(num_epochs):
    ...
    with torch.no_grad():
      ...
      pred = model(x)
      valid_epoch_loss = criterion(pred, y_valid.reshape(-1, 1)).detach().cpu()
      score = scipy.stats.spearmanr(pred.detach().cpu(), y_valid.cpu()).correlation
