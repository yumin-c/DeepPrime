...
from model import ConvTransformer, BalancedMSELoss
...
for fold in range(5):
    ...
    for m in range(n_models):
        ...
        model = ConvTransformer(hidden_size=hidden_size, nhead=nhead, dim_feedforward=dim_feedforward, layer_norm_eps=layer_norm_eps, num_encoder_layers=num_encoder_layers, num_features=num_features, dropout=dropout).to(device)
...
train_file.to_csv('results/convtransformer/final_5cv.csv', index=False)
